...
(function (TEX) {
  ...
  TEX.Stack = MathJax.Object.Subclass({
    Init: function (env) {
      ...
      this.env = (env ? env : this.data[0].env);
    },
    Push: function () {
      var i, m, item, top;
  ...
  var STACKITEM = TEX.Stack.Item = MathJax.Object.Subclass({
    ...
    mmlData: function (forceRow) {
      if (this.data.length === 1 && !forceRow) {return this.data[0]}
      return MML.mrow.apply(MML,this.data).With({inferred: TRUE});
    },
    checkItem: function (item) {
      if (item.type === "over" && this.isOpen) {item.num = this.mmlData(); this.data = []}
...
//        if (mml.type !== "mrow") {mml = MML.mrow(mml).With({inferred:true})}
  ...
  STACKITEM.over = STACKITEM.Subclass({
    ...
    checkItem: function (item,stack) {
      ...
      if (item.isClose) {
        var mml = MML.mfrac(this.num,this.mmlData());
  ...
  STACKITEM.array = STACKITEM.Subclass({
    ...
    checkItem: function (item) {
      if (item.isClose) {
        ...
        if (this.open || this.close) {
          mml = MML.mrow(MML.mspace().With({width:(1/6)+"em"}),mml,MML.mspace().With({width:(1/6)+"em"}));
          mml = MML.mfenced(mml).With({open: this.open, close: this.close});
        }
        mml = STACKITEM.mml(mml);
        if (this.ignoreClose && item.type === 'close') {return mml}
        return [mml,item];
      }
      return this.SUPER(arguments).checkItem.call(this,item);
    },
  ...
  TEX.Startup = function () {
    ...
    MathJax.Hub.Insert(TEXDEF,{
      ...
      remap: {
        '-':   '2212',
        '|':   ['2223',{texClass:MML.TEXCLASS.ORD}],
        '*':   '2217'
      },
	...
	varphi:       '03C6',
      ...
      macros: {
        ...
        Bbb:               ['SetFont',MML.VARIANT.DOUBLESTRUCK],
        scrFont:           ['SetFont',MML.VARIANT.SCRIPT],
        frakFont:          ['SetFont',MML.VARIANT.FRAKTUR],
...
//      fbox:              ['Extension','fbox'],
        ...
        acute:             ['Accent', "0301"],  // or 02CA
        grave:             ['Accent', "0300"],  // or 02CB
        ddot:              ['Accent', "0308"],  // or 00A8
        tilde:             ['Accent', "0303"],  // or 02DC
        bar:               ['Accent', "0304"],  // or 02C9
        breve:             ['Accent', "0306"],  // or 02D8
        check:             ['Accent', "030C"],  // or 02C7
        hat:               ['Accent', "0302"],  // or 02C6
        vec:               ['Accent', "20D7"],
        dot:               ['Accent', "0307"],  // or 02D9
        widetilde:         ['Accent', "0303",1], // or 02DC
        widehat:           ['Accent', "0302",1], // or 02C6
...
//      noalign:            'HandleNoAlign',
        ...
        mod:               ['Macro','\\kern 18mu {\\rm mod}\\,\\,#1',1],
        pod:               ['Macro','\\kern 18mu (#1)',1],
        iff:               ['Macro','\\;\\Longleftrightarrow\\;'],
        skew:              ['Macro','{{#2{#3\\mkern#1mu}\\mkern-#1mu}{}}',3],
        mathcal:           ['Macro','{\\cal #1}',1],
        mathscr:           ['Macro','{\\scrFont #1}',1],
        mathrm:            ['Macro','{\\rm #1}',1],
        mathbf:            ['Macro','{\\bf #1}',1],
        mathbb:            ['Macro','{\\Bbb #1}',1],
        mathit:            ['Macro','{\\it #1}',1],
        mathfrak:          ['Macro','{\\frakFont #1}',1],
        mathsf:            ['Macro','{\\sf #1}',1],
        mathtt:            ['Macro','{\\tt #1}',1],
...
//      bbox:       ['Extension','bbox'],
      ...
      environment: {
        ...
        align:        ['Extension',null,'AMSmath',"environment"],
        'align*':     ['Extension',null,'AMSmath',"environment"],
        aligned:      ['Extension',null,'AMSmath',"environment"],
        multline:     ['Extension',null,'AMSmath',"environment"],
        'multline*':  ['Extension',null,'AMSmath',"environment"],
        split:        ['Extension',null,'AMSmath',"environment"],
        gather:       ['Extension',null,'AMSmath',"environment"],
        'gather*':    ['Extension',null,'AMSmath',"environment"],
        gathered:     ['Extension',null,'AMSmath',"environment"]
       },
  ...
  TEX.Parse = MathJax.Object.Subclass({
    ...
    Prime: function (c) {
      ...
      var sup = "", prime = String.fromCharCode(0x2032);
    ...
    Other: function (c) {
      ...
      this.Push(this.mmlToken(MML.mo(mo).With(def)));
    },
    ...
    LeftRight: function (name) {
      this.Push(STACKITEM[name]().With({delim: this.GetDelimiter("\\"+name)}));
    },
    
    NamedOp: function (name,limits,id) {
      var underover = (limits != null && limits === 0 ? FALSE : TRUE);
      if (!id) {id = name}; limits = ((limits || limits == null) ? TRUE : FALSE);
    ...
    Sqrt: function (name) {
      ...
      if (n == "") {mml = MML.msqrt.apply(MML,mml.array())}
              else {mml = MML.mroot(TEX.Parse(n,this.stack.env).mml(),mml)}
      this.Push(mml);
    },
    
    Root: function (name) {
      var n = this.ParseUpTo("\\"+name,"\\of");
      var arg = this.ParseArg("\\"+name);
      this.Push(MML.mroot(n,arg));
    },
    ...
    UnderOver: function (name,c,stack) {
      var pos = {o: "over", u: "under"}[name.charAt(0)];
    ...
    RaiseLower: function (name) {
      ...
      if (h.charAt(0) === '-') {h = h.slice(1); name = {raise: 'lower', lower: 'raise'}[name]}
    ...
    Rule: function (name,style) {
      ...
      var h = this.GetDimen("\\"+name);
      var d = this.GetDimen("\\"+name);
      var mml = MML.mspace().With({
        width: w, height: h, depth:d, mathsize: MML.SIZE.NORMAL, scriptlevel:0
      });
      if (style !== 'blank') {mml.texfilled = TRUE}
      this.Push(mml);
    },
    ...
    MakeBig: function (name,mclass,size) {
      ...
      this.Push(MML.mrow(MML.mo(delim).With({
    ...
    Extension: function (name,file,array) {
      ...
      if (!MathJax.Ajax.loaded[MathJax.Ajax.fileURL(file)]) {
        if (name != null) {delete TEXDEF[array || 'macros'][name]}
        var callback = MathJax.Ajax.Require(file);
    ...
    */
    ...
    Array: function (begin,open,close,align,spacing,vspacing,style,raggedHeight) {
      ...
      if (lines.match(/\|/))
        {array.arraydef.lines = lines.replace(/[^| ]/g,'none').replace(/\|/g,'solid')}
    ...
    GetDimen: function (name) {
      ...
      if (this.string.charAt(this.i) == '{') {
        ...
        if (dimen.match(/^\s*([-+]?(\.\d+|\d+(\.\d*)?))\s*(pt|em|ex|mu|px)\s*$/))
          {return dimen.replace(/ /g,"")}
      } else {
        var dimen = this.string.slice(this.i);
        var match = dimen.match(/^\s*(([-+]?(\.\d+|\d+(\.\d*)?))\s*(pt|em|ex|mu|px)) ?/);
        if (match) {
    ...
    InternalMath: function (text,level) {
      ...
      while (i < text.length) {
        ...
        if (c === '$') {
          if (match === '$') {
            ...
            mml.push(MML.mtext(MML.chars(text.slice(k,i-1))).With(def));
            match = '$'; k = i;
          }
        } else if (c === '\\') {
          c = text.charAt(i++);
          if (c === '(' && match == '') {
            mml.push(MML.mtext(MML.chars(text.slice(k,i-2))).With(def));
            match = ')'; k = i;
          } else if (c === ')' && match === ')') {
      ...
      mml.push(MML.mtext(MML.chars(text.slice(k))).With(def));
      return mml;
    },
  ...
  TEX.Augment({
    Translate: function (script) {
      ...
      mml.root.displaystyle = displaystyle;
      return mml;
    },
