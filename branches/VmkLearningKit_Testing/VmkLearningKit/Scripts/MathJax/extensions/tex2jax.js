...
MathJax.Extension.tex2jax = {
  ...
  Init: function () {
    ...
    this.PreProcess(this.config.element);
  },
    ...
    while (element) {
      if (element.nodeName.toLowerCase() === '#text') {
        ...
        if (element.firstChild && !cname.match(/(^| )MathJax/) &&
             !tname.match(/^(script|style|textarea|pre)$/i)) {
          ignore = (ignore || cname.match(/(^| )tex2jax_ignore( |$)/)) &&
                    !cname.match(/(^| )tex2jax_process( |$)/);
          this.scanElement(element.firstChild,ignore);
        }
      }
...
MathJax.Hub.Register.PreProcessor(["Init",MathJax.Extension.tex2jax]);
