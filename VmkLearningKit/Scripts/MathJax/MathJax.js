...
(function (BASENAME) {
  ...
  var EVAL = function (code) {
    if (window._content && window._content.eval) {return window._content.eval(code)}
    BASE.__code = code;
  ...
  var SIGNAL = QUEUE.Subclass({
    ...
    ExecuteHooks: function (msg,more) {
      ...
      return HOOKS(this.hooks[type],[msg]);
    }
...
(function (BASENAME) {
  ...
  BASE.Ajax = {
      ...
      checkLength: function (check,node,callback) {
        ...
        try {if ((sheet.cssRules||sheet.rules).length > 0) {isStyle = 1}} catch(err) {
          if (err.message.match(/protected variable|restricted URI/)) {isStyle = 1}
            ...
            //   or OK files in different direcory for local files.
    ...
    loadError: function (file) {
      BASE.Message.Clear(BASE.Message.Set("File failed to load: "+file),1000);
    },
    ...
    if (this.current === n) {
      ...
      if (this.text) {
        if (this.current == null) {
          ...
          this.timer = setTimeout("MathJax.Message.Remove()",(delay||600));
        } else if (this.textNodeBug) {
          this.div.innerHTML = this.log[this.current].text;
        } else {
          this.text.nodeValue = this.log[this.current].text;
        }
        if (this.status) {window.status = ""; delete this.status}
      } else if (this.status) {
...
MathJax.Hub = {
  ...
  Register: {
    ...
    MessageHook: function () {MathJax.Hub.signal.MessageHook.apply(MathJax.Hub.signal,arguments)},
    StartupHook: function () {MathJax.Hub.Startup.signal.MessageHook.apply(MathJax.Hub.Startup.signal,arguments)},
  ...
  checkScriptSiblings: function (script) {
    ...
    if (config.preRemoveClass && pre && pre.className == config.preRemoveClass) {
      try {pre.innerHTML = ""} catch (err) {pre.styles.display = "none"}
    }
...
MathJax.Hub.Startup = {
  ...
  //
  //  Perform any preprocessing hooks that were installed by
  //  the extensions
  //
  PreProcess: function (element,callback) {
    return this.queue.Push(
      ...
      ["ExecuteHooks",MathJax.CallBack,
        (arguments.callee.disabled ? [] : MathJax.Hub.config.preProcessors)],
...
(function (BASENAME) {
  ...
  var JAX = MathJax.Object.Subclass({
    ...
    Translate: function (element) {
      //this.Translate = this.noTranslate; // мое
  ...
  BASE.ElementJax = JAX.Subclass({
    ...
    Text: function (text,callback) {
      ...
      if (script.text != "") {script.text = text} else {script.innerHTML = text}
...
(function (BASENAME) {
  ...
  BASE.CallBack.Queue(
    ...
    ["Extensions",STARTUP],
    STARTUP.onLoad(),
    ["Post",STARTUP.signal,"onLoad"],
    ["Browser",STARTUP],
    ["PreProcess",STARTUP],
    ["Post",STARTUP.signal,"Process"],
    ["Process",HUB],
