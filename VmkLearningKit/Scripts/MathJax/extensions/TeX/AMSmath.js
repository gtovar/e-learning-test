...
MathJax.Hub.Register.StartupHook("TeX Jax Ready",function () {
  ...
  MathJax.Hub.Insert(TEXDEF,{
    macros: {
      ...
      iint:       ['MultiIntegral','\\int\\intI'],
      iiint:      ['MultiIntegral','\\int\\intI\\intI'],
...
//      boxed:      ['Macro','\\fbox{$\\displaystyle{#1}$}',1],
      ...
      varinjlim:  ['Macro','\\mathop{\\smash{\\rlap{\\underrightarrow{\\raise.17em{\\vphantom{\\rm lim}}}}}{\\rm lim}\\rule{0pt}{0pt}{.6em}}'],
      varprojlim: ['Macro','\\mathop{\\smash{\\rlap{\\underleftarrow{\\raise.17em{\\vphantom{\\rm lim}}}}}{\\rm lim}\\rule{0pt}{0pt}{.6em}}'],
    ...
    environment: {
      ...
      'align*':      ['AMSarray',null,true,false, 'rlrlrlrlrlrl',COLS([5/18,2,5/18,2,5/18,2,5/18,2,5/18,2,5/18])],
      multline:      ['Multline',null,true],
      'multline*':   ['Multline',null,false],
      split:         ['AMSarray',null,false,false,'rl',COLS([5/18])],
      gather:        ['AMSarray',null,true,true,  'c'],
      'gather*':     ['AMSarray',null,true,false, 'c'],
  ...
  TEX.Parse.Augment({
    ...
    CFrac: function (name) {
      ...
      var frac = MML.mfrac(TEX.Parse('\\strut\\textstyle{'+num+'}').mml(),
                           TEX.Parse('\\strut\\textstyle{'+den+'}').mml());
    ...
    Genfrac: function (name,left,right,thick,style) {
      ...
      var num = this.ParseArg("\\"+name), den = this.ParseArg("\\"+name);
     ...
     */
    Multline: function (begin,starred) {
      this.Push(begin);
     ...
     */
    AMSarray: function (begin,starred,taggable,align,spacing) {
      this.Push(begin);
      align = align.replace(/[^clr]/g,'').split('').join(' ');
      align = align.replace(/l/g,'left').replace(/r/g,'right').replace(/c/g,'center');
      return STACKITEM.AMSarray(begin.name,starred,taggable,this.stack).With({
        arraydef: {
          displaystyle: true,
          rowspacing: ".5em",
          columnalign: align,
  ...
  STACKITEM.AMSarray = STACKITEM.array.Subclass({
    type: "AMSarray",
    Init: function (name,starred,taggable,stack) {
