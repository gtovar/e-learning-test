...
public partial class Metodist_NewTests_ViewGroupNewTest : System.Web.UI.Page
{
    ...
    {
        for (int i = 0; i < newAssignmentGridView.Rows.Count; i++)
        {
            Label numbers = (Label)newAssignmentGridView.Rows[i].Cells[0].FindControl(Resources.Constants.NumbersLinesLabelName);
            numbers.Text = (i + 1).ToString();
        }
    }
    protected void backToNewTests_Click(object sender, EventArgs e)
    {
        string url = UrlConstants.NewAssignmentsHomeUrl + "?DepartmentID=" + departmentsList.SelectedValue;
